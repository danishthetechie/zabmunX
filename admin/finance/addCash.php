			    ...
			    <div class="row">
					<?php
						...
						if($row = mysqli_fetch_array($run)){
								...
								if($delegation_type == "Double Delegation"){
									...
									//selecting members
									$query2 = "SELECT count(*) as mem_count FROM `delegate_members` where head_del = '$id'";
									$run2 = mysqli_query($con,$query2);
									if($row2 = mysqli_fetch_assoc($run2)){
										$member_count = $row2['mem_count'];
										$total_amount = 3500+(3500 * $member_count);
									}
								}
								else{
										$member_count = 0;
                      ...
                      <div class="card-body card-block">
                        <form action="" method="post" class="form-horizontal">
						  ...
						  <div class="row form-group">
                            ...
                            <div class="col-12 col-md-9"><textarea name="info" id="textarea-input" rows="9" placeholder="Additional Info if any discounts" class="form-control"></textarea></div>
                          </div>
						  <div class="row form-group">
                            <div class="col col-md-3"><label for="text-input" class=" form-control-label">Amount</label></div>
                            <div class="col-12 col-md-9"><input type="text" id="text-input" name="amount" class="form-control"><small class="form-text text-muted">Enter Amount in figures</small></div>
                          </div>
						  <div class="row form-group">
                            <div class="col col-md-3"><label for="text-input" class=" form-control-label">Balance</label></div>
                            <div class="col-12 col-md-9"><input type="text" id="text-input" name="balance" value="0" class="form-control"><small class="form-text text-muted">Keep it 0 if there is no remaining balance</small></div>
                          </div>
						  <div>
					...
					<?php
						if(isset($_POST["submit"])){
							...
							$query3 = "INSERT INTO `payments`(`del_id`, `amount`, `balance`, `total_amount`,`discount_info`, `payment_date`, `status`) VALUES ('$id','$amount','$balance','$total_amount','$info','$date', '$status')";
